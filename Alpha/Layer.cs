...
namespace Game {
    ...
    public class Layer : Drawable {
        ...
        public void PlaceTile(int x, int y, int id) {
            if (id <= 0) return; // 0 is a blank tile
            ...
            int ti = (int) (id % (tileset.Texture.Size.X / tileset.TileSize.X));
            int tj = (int) (id / (tileset.Texture.Size.X / tileset.TileSize.X));
        ...
        public void LoadElement(XElement element) {
            ...
            foreach (var v in element.Descendants("tile")) { // Add each tile to vertex array
                PlaceTile(i % LayerSize.X, i / LayerSize.Y, int.Parse(v.Attribute("gid").Value));
                i++;
            }
        }
    }
